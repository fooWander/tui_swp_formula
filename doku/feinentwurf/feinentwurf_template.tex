...
Mit dem Demultiplexer ist es anschließend möglich, aus dem Inputsignal \textit{vector\_d} bestimmte Elemente zu entnehmen und auszugeben. In diesem Beispiels werden die 3 Elemente des Vektors jeweils wieder einzeln als Skalare herausgeführt und anschließend auf Terminator-Blöcke geleitet. Je nach Anzahl der Inputs \textit{n} und der Outputs \textit{p} des Demultiplexers ergeben sich u.a. die folgenden Kombinationen:
...
\item 
Der virtuelle Server ist vor Kommunikationsbeginn in einer Art Wartezustand in dem er auf eintreffende Pakete wartet, was durch das Horchen an einem Netzwerk-Port durch die Anwendung \textit{vserver} realisiert wird.
[listen on port D]

\item
Die Kommunikation beginnt beim Starten des Embedded-PCs und des darauf folgenden Starts der Anwendung embpc. Der PC ist nach fehlerfreiem Start dazu in der Lage sein erstes Paket zu senden und damit dessen erste Programmanweisung auszuführen. Das Paket (ready\_1) wird über eine UMTS-Verbindung an den virtuellen Server gesendet und beinhaltet einen mit einer bestimmten Bit-Folge bestückten Bitvektor (Länge 8 bit, 1 Byte), welcher die Bereitschaft des Embedded-PCs signalisiert. Nach dem Sendevorgang wartet er auf ein Acknowledgement des vServers. 
[send ready\_1 to port D]

\item
Hat der virtuelle Server diesen Bitvektor erfolgreich empfangen [get ready\_1 from EmbPC] und dekodiert, sendet dieser dem Embedded-PC eine Empfangsbestätigung, wieder in Form eines 8 bit großen Bitvektors bestimmter Kodierung. Wurde nicht der erwartete Bitvektor empfangen wird wie in Schritt 1.1.1 weiter gelauscht. 
[send ready\_2 to port C1]

\item
Falls jedoch keine Empfangsbestätigung eintrifft [listen on port C1] wird, nach einer von uns zuvor festgelegten Zeitdauer (timeout C1) Schritt 1.1.2 wiederholt, solange bis dieser eine korrekte Empfangsbestätigung, ausgehend vom virtuellen Server, erhält. 
[get ready\_2 from vServer]

\item
Nach dem Kommunikationsbeginn und des Sendens der Empfangsbestätigung ist das Programm des Servers bereit ein neues Paket (Info-Packet\_2) zu empfangen welches wichtige Informationen zur Weiterverarbeitung enthält (siehe Abbildung (VERWEIS)). 
[listen on port D]

\item
Zuvor jedoch meldet der Embedded-PC seine Bereitschaft und die des vServers durch das wiederholte Senden eines speziell kodierten Bitvektors an die MicroAutoBox II.
[send ready\_3 to port A]

\item
Ist die Autobox im Betrieb, befindet sie sich wie der virtuelle Server ebenfalls bis zum Empfang eines Paketes im Wartezustand.
[listen on port A]

\item
Wird das Paket ausgehend vom Embedded-PC von der MicroAutoBox II empfangen
[get ready\_3 from EmbPC], wird diese nun versuchen dem PC ein erstes Paket mit zu verarbeitenden Nutzdaten zu übermitteln [send Info-Packet\_1] bis diese eine Empfangsbestätigung (ACK\_1) erhält. 
[listen on port A]

\item
Nach dem Empfang des Paketes [get Info-Packet\_1] und dessen Verarbeitung [receive Info-Paket\_1] auf dem Embedded-PC leitet dieser das empfangene Paket unverändert an den vServer weiter [send Info-Paket\_1], solange bis der PC eine Empfangsbestätigung vom Server entgegen nimmt. Nach dieser Empfangsbestätigung übergibt der Embedded-PC der MicroAutoBox die erwartete Empfangsbestätigung [send ACK\_1 to port A]  bis diese empfangen wird.
[get ACK\_1 from EmbPC]
