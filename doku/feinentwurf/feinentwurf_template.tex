...
Mit dem Demultiplexer ist es anschließend möglich, aus dem Inputsignal \textit{vector\_d} bestimmte Elemente zu entnehmen und auszugeben. In diesem Beispiel werden die drei Elemente des Vektors jeweils wieder einzeln als Skalare herausgeführt und anschließend auf Terminator-Blöcke geleitet. Je nach Anzahl der Inputs \textit{n} und der Outputs \textit{p} des Demultiplexers ergeben sich u.a. die folgenden Kombinationen:
...
\item[1) - 2)] 
Der virtuelle Server sowie die MicroAutoBox II sind nach deren Start in einer Art Wartezustand(\texttt{1/2: receive MSG\_READY}), in dem sie auf die Nachricht \texttt{MSG\_READY} des Embedded-PCs warten. Dabei handelt es sich um einen mit einer bestimmten Bitfolge bestückten Bitvektor der Länge ein Byte. (Siehe Abbildung \ref{paketinhalt}) Dieser signalisiert beiden Teilnehmern die Bereitschaft des Embedded-PCs. 
...
\begin{figure}[h]
\centering
\includegraphics[scale = 0.60]{StartResizeDBsequenz}
\caption[Sequenzdiagramm des Programmes StartResizeDB auf dem vServer]{Sequenzdiagramm des Programmes StartResizeDB auf dem vServer}
\label{resizeDBSequenz}
\end{figure} 

Dieses Sequenzdiagramm beschreibt den Ablauf des Programmes \textit{StartResizeDB} auf dem virtuellem Server. Direkt nach dem Aufruf des Programmes wird die Klasse \textit{resizeDB} erzeugt, welche wiederum über die Klasse \textit{dbData} die Datei \textit{Konfiguration.conf} und somit die Datenbankzugangsdaten einliest.
Mit Hilfe dieser Daten wird nun eine Verbindung zur Datenbank auf dem MySQL-Server aufgebaut.\\
Nachfolgend wird von \textit{resizeDB} für jede Tabelle die aktuelle Anzahl an Datensätzen abgefragt, sollten mehr Einträge als vorgesehen in der Tabelle liegen, werden sie anschließend über ein weiteres MySQL-Statement aus der Datenbank gelöscht.
\textit{resizeDB} gibt nun über die Klasse \textit{logData} in einen Logeintrag in der Datei \textit{log.txt} an, wie viele Datensätze gelöscht wurden.
Falls der Datenbankzugriff fehlschlägt gibt \textit{resizeDB} über seinen Rückgabewert an, dass Fehler aufgetreten sind.
\textit{StartResizeDB} veranlasst nun ebenfalls über die Loggingklasse \textit{logData} eine Ausgabe entsprechender Fehler in der Logdatei.

\newpage

\begin{figure}[h]
\centering
\includegraphics[scale = 0.75]{StartResizeDB}
\caption[Klassendiagramm des Programmes StartResizeDB auf dem vServer]{Klassendiagramm des Programmes StartResizeDB auf dem vServer}
\label{resizeDB}
\end{figure}


Klasse \textit{StartResizeDB}:\\
\textit{StartResizeDB} stellt ein eigenständiges Programm dar, welches die Fahrzeugdaten-Tabellen der MySQL-Datenbank auf die Anzahl ihrer Einträge überprüft, bei Überschreiten einer vordefinierten Anzahl an Einträgen (regulär 36000, wird aus der Datei \textit{Konfiguration.conf} geladen) werden die überflüssigen ältesten Einträge automatisch gelöscht.\\
...
Es ruft die Funktion \textit{resDB()} der Klasse \textit{resizeDB} auf, wodurch die Datenbank bereinigt wird.
Sollte bei der Datenbankverbindung ein Zugriffsfehler auftreten wird über die Klasse \textit{logData} eine Fehlermeldung in die Loggingdatei \textit{log.txt} geschrieben.\\

\newpage

Klasse \textit{resizeDB}:\\
\textit{resizeDB} wird von der Klasse \textit{StartResizeDB} aufgerufen.
Über die Klasse \textit{dbData} werden die Zugangsdaten für den MySQL-Server abgerufen und anschließend eine Verbindung zur Datenbank mittels des \textit{MySQL Connector/C++} aufgebaut.
War der Verbindungsaufbau erfolgreich, werden die entsprechenden Tabellen mit den Fahrzeugdaten nacheinander überprüft, dazu wird die aktuelle Anzahl an Datensätzen in der Tabelle über ein MySQL-Statement ermittelt und mit der Norm (ebenfalls ein Parameter von \textit{dbData}) verglichen.
...
Dieses Programm ist auch von der Kommandozeile manuell ausführbar, etwa um zu Überprüfen ob die Verbindung mit der MySQL-Datenbank erfolgreich hergestellt wird.
Die Ausgaben über den aktuellen Bearbeitungsstand des Programmes erfolgen via \textit{dbData} in die Logdatei \textit{Konfiguration.conf}.\\
\\

Klasse \textit{dbData}:\\
Die Klasse \textit{dbData} wird verwendet um die Zugangsdaten für die MySQL-Datenbank und weitere Programmparameter aus einer Konfigurationsdatei (siehe \textit{Konfiguration.conf}) auszulesen.
Um die Werte zu speichern werden für jeden Parameter entsprechende Variablen angelegt.
Der Konstruktor öffnet bei der Erzeugung eines Objektes dieser Klasse die Datei \textit{Konfiguration.conf} aus dem  gleichem Verzeichnis und durchsucht alle Zeilen nach vordefinierten Schlüsselwörtern.
Wird ein Schlüsselwort gefunden, wird die entsprechende Variable mit dem Inhalt der nächsten Zeile (ggf. in einem anderen Datentyp konvertiert) belegt.
Sollte das Programm eine Zeile nicht deuten können, wird über die Klasse \textit{logData} in der Logdatei ein entsprechender Eintrag angelegt.
Des weiteren stellt diese Klasse Get-Memberfunktionen zum Abrufen der einzelnen Parameter bereit.\\
\\

Konfigurationsdatei \textit{Konfiguration.conf}:\\
Diese Datei bietet die Möglichkeit, Programmparameter nach der Kompilierung des Programms und ggf. auch während der Laufzeit zu verändern.
Dazu wird sie von der Klasse \textit{dbData} ausgelesen. Da diese Datei auch das Datenbankzugangspasswort enthält ist darauf zu achten, dass sie nur für autorisierte Benutzer einsehbar ist.\\
\\

Klasse \textit{logData}:\\
Die Klasse \textit{logData} stellt für andere Klassen eine gemeinsame Loggingfunktion in die Datei \textit{log.txt} zur Verfügung.
Rudimentär ist ebenfalls eine Loglevelfunktionalität enthalten.
Wird die Funktion ohne Loglevelangabe aufgerufen, wird hierfür automatisch das Loglevel sechs festgelegt.
Nur Mitteilungen mit einem Loglevel das größer ist als der vordefinierte Schwellwert drei werden in die Logdatei geschrieben.
Dazu wird die Datei \textit{log.txt} geöffnet und die erhaltene \textit{String}-Nachricht, mit einem Zeitstempel versehen, hineingeschrieben. Sollte die Datei noch nicht existieren wird sie automatisch erstellt.\\
\\

Logdatei \textit{log.txt}:\\
In der Datei textit{log.txt} werden Ausgaben der Klassen \textit{dbData}, \textit{resizeDB}, \textit{StartResizeDB} und \textit{DBPacketInsert} durch die Klasse \textit{logData} gespeichert.
Diese Datei ist für den korrekten Ablauf des Programms nicht notwendig.\\
\\
...
\textbf{$\rightarrow siehe$ \ref{akkudatenjs} akkudaten\_script.js}\\
...
\textbf{$\rightarrow siehe$ \ref{allgemeinefahrzeugdatenjs} allgemeinefahrzeugdaten\_script.js}\\
...
\textbf{$\rightarrow siehe$ \ref{dynamischedatenjs} dynamischedaten\_script.js}\\
...
\textbf{$\rightarrow siehe$ \ref{fahrdynamikjs} fahrdynamik\_script.js}\\
...
\textbf{$\rightarrow siehe$ \ref{motorundumrichterjs} motorundumrichter\_script.js}\\
...
\label{allgemeinefahrzeugdatensql}
...
$\rightarrow$ Für weitere Informationen siehe Feinentwurfsdokument \glqq Online Benutzer\grqq (3.4.31)



%%% ENTWURFSDOKUMENT DAVID %%%
...
\label{allgemeinefahrzeugdatenjs}
...
\hangindent+95pt \hangafter=1
Zeigt den aktuellen Lenkwinkel. Wird durch zwei geteilt und dadurch auf die 181 Abstufungen normiert.\\
...
\begin{figure}[h]
\centering
\includegraphics[scale = 0.8]{DatenUebertragung_Verarbeitung}
\caption[Klassendiagramm über Datenübertragung und Verarbeitung]{Klassendiagramm über Datenübertragung und Verarbeitung}
\label{DatVerarbeitung}
\end{figure}
...
\begin{figure}[h]
\centering
\includegraphics[scale = 0.8]{PracticalSocket}
\caption[Klassendiagramm über Socketerstellung und -benutzung]{Klassendiagramm über Socketerstellung und -benutzung}
\label{PracticalSocket}
\end{figure}
